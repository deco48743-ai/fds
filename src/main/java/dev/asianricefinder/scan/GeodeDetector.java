...
public final class GeodeDetector {
    ...
    public List<GeodeHit> detect(ClientWorld level, WorldChunk chunk, ChunkKey target, int minY, int maxY) {
    ...
    private GeodeHit flood(ClientWorld level, BlockPos start, Set<BlockPos> visited) {
    ...
    private boolean isLoaded(ClientWorld level, BlockPos pos) { return level.getChunkManager().getWorldChunk(pos.getX() >> 4, pos.getZ() >> 4) != null; }
    ...
    private boolean enclosedByDeepslate(ClientWorld level, BlockPos center) {
        int deepslate = 0;
        for (int dx = -1; dx <= 1; dx++) for (int dy = -1; dy <= 1; dy++) for (int dz = -1; dz <= 1; dz++) {
            if (dx == 0 && dy == 0 && dz == 0) continue;
            Block block = level.getBlockState(center.add(dx, dy, dz)).getBlock();
