...
public final class AsianRiceFinderRenderer {
    public void render(Render3DEvent event, Map<ChunkKey, DetectionScore> qualified, int renderDistance, int height, SettingColor side, SettingColor line) {
        ...
        for (Map.Entry<ChunkKey, DetectionScore> entry : qualified.entrySet()) {
            ...
            double dx = MinecraftClient.getInstance().player.getX() - (key.minBlockX() + 8.0);
            double dz = MinecraftClient.getInstance().player.getZ() - (key.minBlockZ() + 8.0);
            if (dx * dx + dz * dz > maxDistanceSquared) continue;
            double middleY = MinecraftClient.getInstance().player.getY();
            Box box = new Box(key.minBlockX(), middleY - height / 2.0, key.minBlockZ(), key.minBlockX() + 16, middleY + height / 2.0, key.minBlockZ() + 16);
